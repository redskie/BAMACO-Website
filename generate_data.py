...
def extract_player_data(html_file):
    ...
    try:
        ...
        def get_value(key):
            # Updated pattern to handle escaped quotes
...
def generate_data_json():
    ...
    # Populate guild members from players and auto-calculate member counts
    ...
    for guild in guilds:
        ...
        # AUTO-CALCULATE MEMBER COUNT based on actual player profiles
        actual_member_count = len(guild_members)
        guild['memberCount'] = actual_member_count
        print(f"  👥 {guild['name']}: {actual_member_count} member(s) (auto-calculated)")
